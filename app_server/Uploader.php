...
class Uploader
{
  ...
  function __construct(){
    ...
    $this->grid_connector = new \GridConnector($this->connect->con, "PDO");
    $this->grid_connector->configure("docs");
//    $this->grid_connector->enable_log("Log",true);
    $this->upload();
    die();
  }
...
//    }
    ...
    */
    ...
    if (@$_REQUEST["mode"] == "html5" || @$_REQUEST["mode"] == "flash") {
      ...
      $action = move_uploaded_file($_FILES["file"]["tmp_name"],Yii::$app->basePath.'/web/docs/'.$filename);
      if($action){
            $this->grid_connector->insert(array(
            "name" => "$filename",
            "file" => "$filename"
        ));
    ...
    */
    ...
    if (@$_REQUEST["mode"] == "html4") {
      ...
      if (@$_REQUEST["action"] == "cancel") {
        ...
        $filename = $_FILES["file"]["name"];
         move_uploaded_file($_FILES["file"]["tmp_name"], Yii::$app->basePath.'/web/docs/'.$filename);
